                    ...
                    @if (count($publish_requests) > 0)
                        @foreach ($publish_requests as $publish_request)
                            <tr data-entry-id="{{ $publish_request->id }}">
                                ...
                                <td><a href="{{asset('uploads/'.$publish_request->player_movie->movie_file)}}">{{$publish_request->player_movie->movie_file}}</a></td>
                                <td>
                                ...
                                <td>
                                    @if($publish_request->is_published == 0)
                                    <a href="{{ route('publish_requests.edit',[$publish_request->id]) }}" class="btn btn-xs btn-info">publish</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
